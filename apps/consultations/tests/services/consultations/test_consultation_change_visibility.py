...
def test_consultation_change_visibility() -> None:
    ...
    result = consultation_change_visibility(consultation=consultation, is_visible=True)
    expected = Consultation.objects.filter(is_visible=True).first()

    assert expected == result
    assert result.is_visible is True

    result = consultation_change_visibility(consultation=consultation, is_visible=False)
